...
def menu(estado):
    ...
    def ver_poblacion():
        ...
        def anterior():
            ...
            images = Image.open(carpeta+"/"+generaciones[0][arbolactual][0]+".bmp")
            smallimg = images.resize((300,300))
            image = ImageTk.PhotoImage(smallimg)
            ...
            nombrelabel.configure(text = generaciones[0][arbolactual][0])
            nombrelabel.update()
        ...
        nombrelabel = Label(master=inicial,text = generaciones[0][arbolactual][0])
        nombrelabel.place(x=260,y=1)
    ...
    def ver_generacion():
        ...
        def ver():
            ...
            def verpadres():
                ...
                imagesm = Image.open(carpeta+"/"+generaciones[int(generacion.get())-1][madre][0]+".bmp")
                ...
                padrelabel = Label(master=padres,text = "Padre")
            ...
            def anterior():
                ...
                images = Image.open(carpeta+"/"+generaciones[int(generacion.get())][arbolactual][0]+".bmp")
                smallimg = images.resize((300,300))
                image = ImageTk.PhotoImage(smallimg)
                ...
                profundidadlabel.update()
                nombrelabel.configure(text = generaciones[int(generacion.get())][arbolactual][0])
                nombrelabel.update()
                hijobtn.place_forget()
                hijol.place_forget()
                hijo = []
...
def elegir_silueta():
    ...
    def seleccionar_img():
        ...
        figura = os.path.basename(archivo)[:-4]
        carpeta = figura
    ...
    def continuar_aux():
        ...
        poblacion.destroy()
        menu(1)
        guardar_datos(generaciones,carpeta)
...
def cargar():
    ...
    def seleccionar_archivo():
        ...
        print(figura)
        carpeta = figura
        with open(("Cargadas/"+figura+".pkl"),"rb") as f:
            generaciones = pickle.load(f)
    def continuar_aux():
        # Lleva al menu de ver poblacion y generacion
        #Recibe el path de la silueta
        poblacion.destroy()
